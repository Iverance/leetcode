...
class Solution(object):
    def shoppingOffers(self, price, special, needs):
        ...
        memo = {}
        def recur(target):
            # original price without special
            val = sum(price[x]*target[x] for x in range(n))
            # iterate thru the specials and memorize
            for spec in special:
                afterSpec = [target[x]-spec[x] for x in range(n)]
                # print(tuple(afterSpec))
                if min(afterSpec) >= 0:
                    val = min(val, spec[-1]+memo.get(tuple(afterSpec), recur(afterSpec)))
            memo[tuple(afterSpec)] = val
            return val
